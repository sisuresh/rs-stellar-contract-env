...
impl Observations {
    fn load(protocol: u32, testname: &str) -> Self {
        ...
        let obs: BTreeMap<String, String> = if path.exists() {
            ...
            let file = File::open(&path).unwrap_or_else(|_| panic!("unable to open {}", path.display()));
            serde_json::from_reader(file).unwrap_or_else(|_| panic!("failed to parse {}", path.display()))
        } else {
            BTreeMap::new()
        };
        Self(obs)
    ...
    fn save(&self, protocol: u32, testname: &str) {
        ...
        let file = File::create(&path).unwrap_or_else(|_| panic!("unable to create {}", path.display()));
