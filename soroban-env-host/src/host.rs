...
impl Host {
    ...
    fn call_n(
        ...
        if self.is_debug() {
            self.fn_call_diagnostics(contract, func, args)?;
        }
                ...
                return self.with_frame(Frame::TestContract(frame), || {
                    ...
                    match res {
                        Ok(Some(rawval)) => {
                            ...
                            self.fn_return_diagnostics(contract, func, rawval)?;
                            Ok(rawval)
                        },
        ...
        if self.is_debug() {
            match res.clone() {
                Ok(res) => self.fn_return_diagnostics(contract, func, res)?,
                Err(err) => {}
            }
        }
        return res;
...
impl EnvBase for Host {
    ...
    fn bytes_copy_from_slice(
        ...
        self.add_host_object(vnew).map(|ev| ev.into())
    }
    ...
    fn bytes_new_from_slice(&self, mem: &[u8]) -> Result<Object, HostError> {
